...
const FundsDialog: React.FC<FundsDialogProps> = ({
  ...
  const handleFindByStudentNumber = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from('users').select('*').eq('student_number', studentNumber).single();
      if (error) {
    ...
    return <div className="space-y-4 my-4">
        ...
        <div className="grid grid-cols-4 items-center gap-4">
          ...
          <div className="col-span-3 flex gap-2">
            <Input id="pinCode" type="password" value={pinCode} onChange={e => setPinCode(e.target.value)} placeholder="Enter override PIN" className="flex-1" />
            <Button onClick={handlePinSubmit} disabled={!pinCode} className="shrink-0">
              Override
            </Button>
          </div>
        </div>
  ...
  const renderPinNotification = () => {
    ...
    return <Alert className="mb-4 bg-green-50 border-green-200">
  ...
  return <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <div className="grid gap-4 py-4">
          {!readOnlyId && <>
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <div className="col-span-3 flex gap-2">
                  <Input id="studentNumber" value={studentNumber} onChange={e => setStudentNumber(e.target.value)} placeholder="Enter student number" className="flex-1" />
                  <Button variant="outline" onClick={handleFindByStudentNumber} disabled={isSearching || !studentNumber.trim()} className="shrink-0">
                    {isSearching ? 'Searching...' : 'Find'}
                  </Button>
                </div>
              </div>

              {foundStudent && <div className="px-4 py-2 rounded-md bg-muted/50">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Input id="studentId" value={localStudentId} onChange={e => setLocalStudentId(e.target.value)} className="col-span-3" readOnly={readOnlyId || foundStudent !== null} />
          </div>
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Input id="amount" type="number" min="0.01" step="0.01" value={amount} onChange={e => setAmount(e.target.value)} className="col-span-3" />
          </div>
        ...
        <DialogFooter>
          ...
          <Button variant={confirmVariant} onClick={handleSubmit} disabled={!localStudentId || !amount || isNaN(parseFloat(amount)) || parseFloat(amount) === 0 || isRefund && !reason.trim() && !showPinInput}>
