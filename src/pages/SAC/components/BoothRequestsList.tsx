...
const BoothRequestsList = ({ boothRequests = [], refresh }: BoothRequestsListProps) => {
    ...
    const [selectedFilter, setSelectedFilter] = useState<"all" | "approved" | "pending">("pending");
    ...
    const handleClickApprove = async (idx: number) => {
        if (!displayApproveConfirmation[idx]) {
            ...
            try {
                ...
                let pin : string = Math.floor(100000 + Math.random() * 900000).toString();
                let boothsPINQuery = query(boothsCollection, where('pin', '==', pin));
                let boothsPINSnapshot = await getDocs(boothsPINQuery);
                let count = 0;

                while (!boothsPINSnapshot.empty && count < 3) {
                    pin = Math.floor(100000 + Math.random() * 900000).toString();
                    boothsPINQuery = query(boothsCollection, where('pin', '==', pin));
                ...
                if (allGood) {
                    ...
                    const boothRequestRef = doc(firestore, "booth_requests", boothRequests[idx].id)
                ...
                <div className="w-full h-full flex flex-col gap-y-2">
                    <div className="flex justify-between">
                        <div className="w-full flex justify-start gap-x-2">
                            <button onClick={() => setSelectedFilter("all")} style={{ color: selectedFilter === "all" ? "white" : "black", backgroundColor: selectedFilter === "all" ? "black" : "white" }} className="font-bold p-2 border-2 border-white hover:border-black rounded-lg transition-all duration-200">All</button>
                            <button onClick={() => setSelectedFilter("pending")} style={{ color: selectedFilter === "pending" ? "white" : "black", backgroundColor: selectedFilter === "pending" ? "#CA8A04" : "white" }} className="font-bold p-2 border-2 border-white hover:border-yellow-600 rounded-lg transition-all duration-200">Pending</button>                    
                            <button onClick={() => setSelectedFilter("approved")} style={{ color: selectedFilter === "approved" ? "white" : "black", backgroundColor: selectedFilter === "approved" ? "#33A349" : "white" }} className="font-bold p-2 border-2 border-white hover:border-green-600 rounded-lg transition-all duration-200">Approved</button>
                        </div>
                    ...
                    </div>
                    {
                        boothRequests.length > 0 ?
                            ...
                            .filter(({ i }) =>
                                (selectedFilter === "approved" && approved[i]) || 
                                (selectedFilter === "pending" && !approved[i]) || 
                                (selectedFilter === "all"))
                            .map(({ boothRequest, i }) => (
                                <Card key={i}>
                                    <div className="md:flex md:justify-between md:pr-4">
                                        <CardHeader className="md:w-2/3">
                                            <CardTitle className="flex place-items-center gap-x-4">{boothRequest.boothName} <div className="w-2 h-2 rounded-full" style={{ display: selectedFilter === "all" ? "block" : "none", backgroundColor: approved[i] ? "#33A349" : "#CA8A04" }}></div></CardTitle>
                                            <CardDescription className="overflow-ellipsis">{boothRequest.boothDescription}</CardDescription>
                                        </CardHeader>
                                        <div className="flex flex-row md:flex-col justify-evenly place-items-end text-sm">
                                            <button disabled={approveLoading[i]} onClick={() => handleClickApprove(i)} style={{ display: approved[i] ? "none" : "flex", cursor: approveLoading[i] ? "wait" : "pointer" }} className="hover:text-green-500 flex place-items-center flex-col md:flex-row p-1">{displayApproveConfirmation[i] && <div className="font-bold">Are you sure? </div>}<Check /></button>
                                            <button onClick={() => handleClickInspect(i)} className="hover:text-gray-400 p-1"><ScanEye /></button>
                                        </div>
                                    </div>
                                   {/* INSPECTION MODAL */}
                                    <div
                                    ...
                                    >
                                        ...
                                        <div
                                            className={`relative bg-white rounded-xl text-black p-4 w-full max-w-[600px] transition-all duration-500 ease-in-out
                                                        ${activeInspectModal === i
                                                        ? "opacity-100 scale-100 translate-y-0"
                                                        : "opacity-0 scale-95 translate-y-4"}`}
                                        >
                                            <div className="flex justify-between items-center">
                                                <div className="font-semibold">Inspecting booth "{boothRequest.boothName}"</div>
                                                <button
                                                    className="hover:text-red-500 transition-all duration-200"
                                                    onClick={() => setActiveInspectModal(-1)}
                                                >
                                                    <X />
                                                </button>
                                            </div>
                                            <div className="text-sm flex flex-col place-items-start gap-y-2 pt-4 border-t-2 border-t-gray-500">
                                                <div><b>Booth name: </b>{boothRequest.boothName}</div>
                                                <div><b>Booth description: </b>{boothRequest.boothDescription}</div>
                                                ...
                                                <ul className="pl-4 list-disc">
                                                    {
                                                        boothRequest.teachers.map((teacher, i) => (
                                                            <li key={i}>{teacher.name} | {teacher.email}</li>
                                                        )) 
                                                    }
                                                </ul>
                                                ...
                                                <ul className="pl-4 list-disc">
                                                    {
                                                        boothRequest.products.map((product, i) => (
                                                            <li key={i}>{product.name} | ${(product.price/100).toFixed(2)}</li>
                                                        ))
                                                    }
                                                </ul>
                                                <div className="text-sm">Additional Information: {boothRequest.additionalInformation || "None provided"}</div>
                                            </div>
                                        </div>
                                    </div>
                                    {/* APPROVAL MODAL */}
                                    ...
                                    >
                                        ...
                                        <div
                                            className={`relative bg-white rounded-xl text-black p-4 w-full max-w-[600px] transition-all duration-500 ease-in-out
                                                        ${activeApproveModal === i ?
                                                        "opacity-100 scale-100 translate-y-0" :
                                                        "opacity-0 scale-95 translate-y-4"}`}
                                        >
                                            <div className="flex justify-between items-center">
                                                <div className="font-semibold">Approving booth "{boothRequest.boothName}"</div>
                                                <button
                                                    className="hover:text-red-500 transition-all duration-200"
                                                    style={{ display: approveLoading[i] ? "hidden" : "block" }}
                                            ...
                                            <div className="text-sm flex flex-col place-items-start gap-y-2 pt-4 border-t-2 border-t-gray-500">
                                                <div className="">
                                                    ...
                                                    {
                                                        foundUniquePIN ? 
                                                        <div><b>Unique PIN found ✅: </b>{PIN}</div> : 
