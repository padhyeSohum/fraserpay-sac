...
const BoothSettings = () => {
  ...
  const handleAddProduct = async () => {
    ...
    try {
      console.log('Adding product to booth:', {
        boothId: booth.id,
        product: newProduct
      });
        ...
        <TabsContent value="settings" className="animate-fade-in mt-6">
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                {booth.products && booth.products.length > 0 ? <div className="space-y-2">
                    {booth.products.map(product => <div key={product.id} className="flex justify-between items-center py-2 border-b border-border/30 last:border-0">
                        <div>
                          <div className="font-medium">{product.name}</div>
                          <div className="text-sm text-muted-foreground">${product.price.toFixed(2)}</div>
                        </div>
                        
                        <Button variant="ghost" size="icon" onClick={() => handleRemoveProduct(product.id)} className="text-muted-foreground hover:text-destructive">
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input id="name" placeholder="e.g., Pizza Slice" value={newProduct.name} onChange={e => setNewProduct({
              ...newProduct,
              name: e.target.value
            })} />
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="price" type="number" placeholder="0.00" step="0.01" min="0.01" value={newProduct.price} onChange={e => setNewProduct({
                ...newProduct,
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowAddProductDialog(false)} disabled={isAddingProduct}>
              Cancel
            </Button>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDeleteBooth} disabled={isDeleting} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
              {isDeleting ? <>
                  <span className="mr-2 h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent"></span>
                  Deleting...
                </> : 'Delete Booth'}
